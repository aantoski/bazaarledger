...
export function useListEntries() {
  ...
  useEffect(() => {
    async function fetchAll() {
      const [itemsRes, processedRes, recipesRes, pgiRes] = await Promise.all([
        ...
        // supabase.from('processed_materials').select('processed_good_id, item_id, input_good_id, ingredient_group'),
        // supabase.from('recipe_ingredients').select('recipe_id, item_id, input_good_id, ingredient_group'),
        supabase.from('processed_materials')
          .select('processed_good_id, item_id, input_good_id, ingredient_group')
          .limit(10000),
        // supabase.from('recipe_ingredients')
        //   .select('recipe_id, item_id, input_good_id, ingredient_group')
        //   .range(0, 9999),
      ])
      ...
      console.log('riRes sample:', riData?.slice(0, 3))
      console.log('pgiRes sample:', pgiRes.data?.slice(0, 3))

      // ── Step 1: Build chain maps (must come first) ────────────────────────
      ...
      // ── Step 3: Define helpers AFTER all maps are built ───────────────────
      ...
      // // machine color map: processed_good_id -> machine_color
      // const goodMachineMap: Record<number, string | null> = {}
      // for (const p of processedRes.data ?? []) {
      //   goodMachineMap[p.id] = p.machine_color ?? null
      // }
      ...
      // // ── Step 2: Define recursive helpers (after maps are built) ──────────
      // function collectItemIds(goodId: number, visited = new Set<number>()): number[] {
      //   if (visited.has(goodId)) return []
      //   visited.add(goodId)
      //   const direct = pgiDirectMap[goodId] ?? []
      ...
      // ── Step 3: Build processed good ingredient groups + machine maps ─────
      ...
      for (const row of pgiRes.data ?? []) {
        ...
        if (row.item_id) pgiGroupTemp[row.processed_good_id][group].push(row.item_id)
        if (row.input_good_id) {
      ...
      // ── Step 4: Build recipe ingredient groups + machine maps ─────────────
      ...
      for (const row of riData ?? []) {
        // console.log('ri row:', row.recipe_id, row.input_good_id)
        ...
        if (row.item_id) riGroupTemp[row.recipe_id][group].push(row.item_id)
        if (row.input_good_id) {
      ...
      console.log('Golden Blend Tea ri rows:', riData?.filter(r => r.recipe_id === 218))
      // console.log('Golden Blend Tea entry wonderstone:', riWonderstoneMap[/* whatever ID comes back */])
